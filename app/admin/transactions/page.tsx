...
function TransactionsContent() {
    ...
    const { 
        activeTab, setActiveTab, transactions, totalTransactions, currentPage, setCurrentPage, totalPages,
        ...
        handleApplyFilter, handleResetFilter, handleDownloadExcel 
    ...
    return (
        <>
            ...
            <div className="bg-white p-4 md:p-5 rounded-xl border border-[#E5E7EB] flex flex-col xl:flex-row gap-4 md:gap-5 items-start xl:items-end shadow-sm">
                <div className="w-full xl:w-1/4"><label className="block text-xs font-bold text-gray-600 mb-2">Urutkan Mobil</label><select value={sortOrder} onChange={(e) => setSortOrder(e.target.value as any)} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-600 bg-gray-50/50 outline-none focus:border-[#415A77] transition-colors"><option value="newest">Tanggal (Terbaru)</option><option value="asc">Nama Mobil (A - Z)</option><option value="desc">Nama Mobil (Z - A)</option></select></div>
                <div className="w-full xl:w-1/4"><label className="block text-xs font-bold text-gray-600 mb-2">Status Stok</label><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as any)} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-600 bg-gray-50/50 outline-none focus:border-[#415A77] transition-colors"><option value="all">Semua Status</option><option value="Tersedia">Hanya Tersedia</option><option value="Terjual">Hanya Terjual</option></select></div>
                
                <div className="flex flex-col md:flex-row items-start md:items-end gap-3 w-full xl:w-auto xl:flex-1"><div className="w-full md:w-auto flex-1"><label className="block text-xs font-bold text-gray-600 mb-2">Rentang Tanggal</label><div className="flex items-center gap-2"><input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full md:w-[135px] border border-gray-300 rounded-lg px-3 py-2.5 text-sm text-gray-600 bg-gray-50/50 outline-none focus:border-[#415A77] transition-colors" /><span className="text-gray-400 font-medium">-</span><input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full md:w-[135px] border border-gray-300 rounded-lg px-3 py-2.5 text-sm text-gray-600 bg-gray-50/50 outline-none focus:border-[#415A77] transition-colors" /></div></div><div className="flex items-center gap-2 w-full md:w-auto mt-1 md:mt-0"><button onClick={handleResetFilter} title="Reset Filter" className="flex items-center justify-center bg-gray-50 border border-gray-200 text-gray-600 px-3 py-2.5 rounded-lg hover:bg-gray-200 hover:text-gray-800 transition-colors shadow-sm shrink-0"><RefreshCw size={18} /></button><button onClick={handleApplyFilter} className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-white border border-gray-300 text-gray-700 px-5 py-2.5 rounded-lg text-sm font-bold hover:bg-gray-50 transition-colors shadow-sm shrink-0"><Filter size={16} /> Filter</button></div></div>
            </div>
                                ...
                                <tr key={trx.dbId} className="hover:bg-gray-50 transition-colors cursor-pointer group" onClick={() => router.push(`/admin/transactions/detail/${activeTab}/${trx.dbId}`)}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap flex items-center gap-3">
                                        {trx.coverUrl && isImage(trx.coverUrl) ? (
                                            ...
                                            <div className="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400 shrink-0 border border-gray-200"><CarFront size={20} /></div>
                                        )}
                                        <div>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wider ${trx.status === 'Tersedia' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                                            {trx.status}
                                        </span>
                                    </td>
                ...
                {totalPages > 1 && (
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 border-t border-gray-200 bg-gray-50/50">
                        <span className="text-xs font-medium text-gray-500">
                            Menampilkan halaman <b className="text-[#1B263B]">{currentPage}</b> dari <b className="text-[#1B263B]">{totalPages}</b> ({totalTransactions} data)
                        </span>
                        <div className="flex gap-2">
                            <button 
                                disabled={currentPage === 1} 
                                onClick={() => setCurrentPage(p => p - 1)} 
                                className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-gray-300 bg-white text-xs font-bold text-gray-700 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-gray-50 transition-colors"
                            >
...
export default function TransactionsPage() {
    return (
        <div className="flex flex-col gap-6 pb-20">
            <Suspense fallback={<div className="p-20 text-center font-bold text-gray-500">Memuat halaman transaksi...</div>}>
                <TransactionsContent />
            </Suspense>
        </div>
    );
