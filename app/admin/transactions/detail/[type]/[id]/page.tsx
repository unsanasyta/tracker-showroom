...
export default function DetailTransactionPage() {
    ...
    return (
        <div className="flex flex-col gap-6 pb-10 print:pb-0 print:gap-3">
            ...
            {/* TOP BAR */}
            <div className="bg-white p-4 md:p-6 rounded-xl border border-[#E5E7EB] shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6 print:border-none print:shadow-none print:p-0 print:mb-2">
                <div>
                    ...
                    <p className="text-[11px] md:text-xs text-gray-500 mt-1">Dicatat pada: {formatDate(data.created_at)}</p>
                </div>
                <div className="flex flex-col sm:flex-row items-center gap-3 print:hidden w-full md:w-auto">
                    <Link href="/admin/transactions" className="w-full sm:w-auto flex justify-center items-center gap-2 px-6 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg text-sm font-bold hover:bg-gray-50 transition-colors shadow-sm">
                        <ArrowLeft size={18} /> Kembali
                    </Link>
                    <button 
                        onClick={handleDownloadPDF}
                        className="w-full sm:w-auto flex justify-center items-center gap-2 px-6 py-2.5 bg-[#1B263B] text-white rounded-lg text-sm font-bold hover:bg-[#0F172A] transition-colors shadow-sm"
                    >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 print:flex print:flex-col print:gap-3">
                    <div className="lg:col-span-8">
                        <div className="bg-white p-4 md:p-6 rounded-xl border border-[#E5E7EB] shadow-sm h-full print:border-gray-200 print:shadow-none print:p-4">
                            ...
                            {/* Responsif 1 kolom HP, 2 kolom Desktop */}
                    ...
                    <div className="lg:col-span-4">
                        <div className="bg-white p-4 md:p-6 rounded-xl border border-[#E5E7EB] shadow-sm flex flex-col h-full gap-4 md:gap-5 print:border-gray-200 print:shadow-none print:p-4 print:gap-3">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-wider print:mb-0.5">Harga Jadi Total</label>
                                <div className="w-full border border-blue-200 rounded-lg px-4 py-3.5 print:py-2 text-base md:text-lg print:text-base font-bold bg-[#EEF4FF] text-[#1B263B]">
                                    Rp {formatRp(data.total_acquisition_cost)}
                                </div>
                            </div>
                            <div className="flex flex-col flex-1">
                                <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase tracking-wider print:mb-0.5">Deskripsi Tambahan</label>
                                <div className="w-full flex-1 border border-gray-200 rounded-lg px-4 py-3 print:py-2 text-xs md:text-sm bg-gray-50 text-gray-700 whitespace-pre-wrap min-h-[100px] print:min-h-0">
                    ...
                    <div className="lg:col-span-8">
                        <div className="bg-white p-4 md:p-6 rounded-xl border border-[#E5E7EB] shadow-sm h-full print:border-gray-200 print:shadow-none print:p-4 print:break-inside-avoid">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5 print:gap-2">
                                ...
                                <div className="col-span-1 sm:col-span-2 pt-2 md:pt-4 print:pt-2 border-t border-gray-100">
                    ...
                    <div className="lg:col-span-4 print:hidden">
                        <div className="bg-white p-4 md:p-6 rounded-xl border border-[#E5E7EB] shadow-sm flex flex-col h-full">
                            <h3 className="text-base md:text-lg font-bold text-[#1B263B] mb-4 border-b pb-3">Dokumen Pendukung</h3>
                            <div className="flex flex-col gap-3 flex-1 overflow-y-auto">
                                {data.document_urls && data.document_urls.length > 0 ? (
                                            ...
                                            return (
                                                <div key={index} className="flex items-center justify-between p-3 bg-blue-50/50 border border-blue-100 rounded-lg group">
                                                    <div className="flex items-center gap-3 overflow-hidden">
                                                        ...
                                                        <span className="text-[10px] md:text-xs text-blue-800 font-medium truncate">{fileName}</span>
                                                    </div>
                                                    <div className="flex items-center gap-1 shrink-0">
                                                        <a href={url} target="_blank" rel="noopener noreferrer" className="p-1.5 text-blue-400 hover:text-blue-700 hover:bg-blue-100 rounded-md transition-colors"><ExternalLink size={16} /></a>
                                                        <button onClick={() => handleDownloadFile(url, fileName)} className="p-1.5 text-blue-400 hover:text-blue-700 hover:bg-blue-100 rounded-md transition-colors"><Download size={16} /></button>
                                                    </div>
                                                </div>
                                            )
                                        })}
                                    ...
                                    <div className="text-center text-xs md:text-sm text-gray-400 py-6 italic border-2 border-dashed border-gray-100 rounded-xl">
                                        Tidak ada dokumen.
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="bg-white p-4 md:p-6 rounded-xl border border-[#E5E7EB] shadow-sm w-full print:border-gray-200 print:shadow-none print:p-4">
                        ...
                        <div className="flex flex-col gap-4 md:gap-6 print:gap-3">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5 print:gap-2 pt-2 md:pt-4 print:pt-2 border-t border-gray-100">
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 mb-1.5 print:mb-0.5 uppercase tracking-wider">Harga Jual Akhir</label>
                                    <div className="w-full border border-gray-200 rounded-lg px-4 py-3 print:py-2 text-base md:text-lg print:text-base font-bold bg-gray-50 text-[#1B263B]">
                                        Rp {formatRp(data.sell_price)}
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 mb-1.5 print:mb-0.5 uppercase tracking-wider">Keuntungan Bersih</label>
                                    <div className="w-full border border-green-200 rounded-lg px-4 py-3 print:py-2 text-base md:text-lg print:text-base font-bold bg-green-50 text-green-700 print:border-gray-300 print:bg-white print:text-black">
                                        Rp {formatRp(data.net_profit)}
                                    </div>
                                </div>
                            </div>
                            <div className="pt-2 print:pt-1">
                                <label className="block text-xs font-bold text-gray-500 mb-1.5 print:mb-0.5 uppercase tracking-wider">Catatan Penjualan</label>
                                <div className="w-full border border-gray-200 rounded-lg px-4 py-3 print:py-2 text-xs md:text-sm bg-gray-50 text-gray-700 min-h-[80px] print:min-h-0">
...
function DetailItem({ label, value, highlight = false }: { label: string, value: string | number | undefined, highlight?: boolean }) {
    return (
        <div>
            ...
            <div className={`w-full border rounded-lg px-3 md:px-4 py-2 md:py-2.5 print:py-1 text-xs md:text-sm font-semibold ${highlight ? 'bg-blue-50/50 border-blue-200 text-blue-800 print:bg-white print:border-gray-300 print:text-black' : 'bg-gray-50 border-gray-200 text-[#1B263B] print:bg-white'}`}>
                {value || '-'}
            </div>
        </div>
    );
